...
function useApiData(
  ...
  const [selectedItemsCache, setSelectedItemsCache] = useState<Map<string, SearchSelectOption>>(new Map());
  ...
  const searchFunction = async (query: string) => {
    ...
    try {
      setIsSearching(true);
      const rawResults = await apiClient.searchOptions(query.trim(), limit);
      const options = rawResults.map(item => apiClient.transformToOption(item));
  ...
  const loadSelectedItem = async (value: string) => {
    ...
    try {
      ...
      if (item) {
        ...
        setSelectedItemsCache(prev => new Map(prev).set(value, option));
      }
    } catch (error) {
...
export default function SearchSelect(props: SearchSelectProps) {
  ...
  const availableOptions = getAvailableOptions(options, searchResults, searchQuery);
  ...
  const selectedOptions = (() => {
    ...
    return values.map(val => {
      ...
      if (options) {
        return options.find((opt: SearchSelectOption) => opt.value === val);
      }
    ...
    >
      ...
      <div
        ...
        className={`
          ...
          border border-border rounded-md shadow-sm cursor-text
          ${disabled ? 'bg-muted cursor-not-allowed' : 'bg-background hover:border-accent'}
              ...
              {multiSelect ? (
                selectedOptions.map(option => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      className="ml-1 hover:bg-accent-foreground hover:text-accent rounded-full p-0.5"
                      onClick={(e) => {
                        e.stopPropagation();
        ...
        <div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg">
          ...
          <div className="p-2 border-b border-border">
            <input
              ref={inputRef}
              type="text"
              className="w-full px-3 py-2 text-sm border border-border rounded bg-popover focus:outline-none focus:ring-2 focus:ring-ring focus:border-ring"
              placeholder="Search..."
              value={searchQuery}
          ...
          >
            ...
            {availableOptions.map((option, index) => {
              const isSelected = multiSelect
                ? (currentValue as string[] | undefined)?.includes(option.value)
