...
test(
    'guest cannot join a second lobby while still a participant in another active session',
    function (): void {
        $guest = User::factory()->guest()->create();
...
test(
    'guest may rejoin the same lobby they already participate in when the room is full',
    function (): void {
        $guest = User::factory()->guest()->create();
